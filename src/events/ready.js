module.exports = async (client) => {
  ...
  const activities = [
    ...
    { name: '@SpecialBot', type: 'LISTENING' }
  ];
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`SpecialBot has left ${guild.guild_name}`);
  }

  client.logger.info('SpecialBot is now online');
  client.logger.info(`SpecialBot is running on ${client.guilds.cache.size} server(s)`);
};
